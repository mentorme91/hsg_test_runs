...
class StorageService {
  ...
  // upload a document to firebase
  Future<bool> uploadLectureNotes(File file, String title, String type,
      String school, String courseCode) async {
    try {
      // Upload file to Firebase Storage
      final Reference storageReference =
  ...
  // capture an image
  Future<String?> captureAnImage(String use, String name) async {
    final ImagePicker picker = ImagePicker();
    final XFile? pickedFile =
        await picker.pickImage(source: ImageSource.gallery);
    ...
    if (pickedFile != null) {
      ...
      try {
        await storage
            .ref('images/${use}photofor${name}.png')
            .putFile(imageFile);
